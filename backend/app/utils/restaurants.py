...
def calculate_restaurant_weighted_rating(fetched_restaurant, mean_rating):
    rating = fetched_restaurant['Rating']
    rating_num = fetched_restaurant['User Ratings Total']
    weighted_rating = (rating * rating_num + mean_rating * MIN_RATINGS_THRESHOLD) / (rating_num + MIN_RATINGS_THRESHOLD)
    return weighted_rating


def calculate_restaurants_weighted_rating(fetched_restaurants):
    try:
        scaler = MinMaxScaler()
        restaurants_weighted_rating = []
        mean_rating = np.mean([fetched_restaurant['Rating'] for fetched_restaurant in fetched_restaurants])
        for fetched_restaurant_json in fetched_restaurants:
        ...
        # assert scaled_restaurants_weighted_rating.shape == (len(fetched_restaurants), 1)
...
def calculate_restaurants_distance_compatibility(fetched_restaurants, current_location):
    try:
        scaler = MinMaxScaler()
        ...
        # assert scaled_restaurants_distance_compatibility.shape == (len(fetched_restaurants), 1)
        return scaled_restaurants_distance_compatibility
    except Exception as e:
        return np.repeat(0.5, len(fetched_restaurants)).reshape(-1, 1)


def calculate_restaurants_budget_compatibility(fetched_restaurants, input_user_budget=BASE_USER_BUDGET_LEVEL):
    try:
        restaurant_price_level = [fetched_restaurant['Price Level'] or 2.5 for fetched_restaurant in fetched_restaurants]
